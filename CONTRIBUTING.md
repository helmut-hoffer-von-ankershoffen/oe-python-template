...
tests/oe_python_template/ # Tests
...
# Run the CLI
docker run --env THE_VAR=THE_VALUE -t oe-python-template --target all --help    # target with all extras
docker run --env THE_VAR=THE_VALUE -t oe-python-template --target slim --help   # slim flavor, no extras
```
