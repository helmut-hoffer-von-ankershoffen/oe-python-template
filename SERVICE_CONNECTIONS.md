...
2. Go to Uptime > Monitors and create a monitor, pointing to the `/api/v1/healthz` endpoint of your API on
   ...
   Vercel Serverless Function the URL should be https://oe-python-template.vercel.app/api/v1/healthz. Copy the URL into the "URL to monitor" field. (b) Set "Alert us when" to "URL returns HTTP status other then",
...
3. Set "What service will this heartbeat track? to "OE Python Template / Scheduled tests"
...
7. Goto https://github.com/helmut-hoffer-von-ankershoffen/oe-python-template/settings/secrets/actions/new and create a new repository secret called `BETTERSTACK_HEARTBEAT_TEST_SCHEDULED_URL`, pasting the URL from your clipboard as value.
