...
def test_cli_fails_on_invalid_setting_with_env_arg() -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "oe_python_template.cli",
            "system",
            "info",
            "--env",
            "OE_PYTHON_TEMPLATE_LOG_LEVEL=FAIL",
        ],
        capture_output=True,
        text=True,
        check=False,
...
def test_cli_fails_on_invalid_setting_with_environ(runner) -> None:
    ...
    with runner.isolated_filesystem():
        ...
        env = os.environ.copy()
        env["OE_PYTHON_TEMPLATE_LOG_LEVEL"] = "DEBUG"
        ...
        # Set environment variable for the test
        env = os.environ.copy()
        env["OE_PYTHON_TEMPLATE_LOG_LEVEL"] = "FAIL"
...
if find_spec("nicegui"):
    ...
    def test_cli_gui_run(runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("oe_python_template.utils._gui.gui_register_pages", mock_gui_register_pages)
        ...
        assert mock_ui_run_args["title"] == "OE Python Template", "title parameter is incorrect"
